...
debugger;
				...
				if ( excess > 0 ) {
					this.y += this.upwardspeed - excess-20;
					this.upwardspeed = 0;
					this.falling = 0;
			...
			// dropping.
			//debugger;
			var excess 	= this.player_collide_with_wall( 3 , 0.8 +20) ;
			var excess2 = this.player_collide_with_wall( 3 , 3.2 +20) ; 
			...
			for (var i=level.triggers.length - 1 ; i >= 0 ; i-- ) {
				...
				if ( this.x >= object.x && this.x <= object.x + object.width && 
					...
					if ( object.name == "trigger" ) {
						...
						let x0 = parseInt(level.getObjCustomPropertyValue(object, 'x0'));
						let x1 = parseInt(level.getObjCustomPropertyValue(object, 'x1'));
						if(this.direction == 0 && this.x <= object.x + this.width) {
							camera.x = x0;
							this.x = this.x - 20;
						} else if(this.direction == 1 && this.x >= object.x) {
							camera.x = x1;
							this.x = this.x + 10;
						}
					}
